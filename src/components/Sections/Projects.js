    ...
    return (
        <section id="projects">
            <div className="py-24 max-w-5xl m-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex">
                    <ul className="items-center w-full py-8 overflow-scroll hidden sm:flex">
                        {
                            data.map(client => {
                                return (
                ...
                <Carousel showArrows={false} autoPlay={true} showStatus={false} infiniteLoop={true}>
                    {
                        data.map(client => {
                            return (
                                <div key={client.client} className="px-3 md:px-4 flex-none shadow-2x">
                                    <figure className="bg-cover m-auto bg-center shadow-lg rounded-xl flex-none w-72 h-96 md:w-xl bg-indigo-600" style={{ backgroundImage: `linear-gradient(180deg, rgba(0, 0, 0, 0.09) 0%, rgba(0, 0, 0, 0.46) 100%), url(${client.bg})` }}>
                                        <figcaption className="flex items-center flex-col justify-center space-x-4 p-6 h-96 md:px-10 md:py-6 bg-gradient-to-br rounded-xl leading-6 font-semibold text-white">
                                            {
                                                client.logo ? <img src={client.logo} /> : <></>
                                            }
